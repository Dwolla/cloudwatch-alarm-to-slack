...
export const fetch = async (a: Alarm): Promise<string> => {
  ...
  const r = await asyncRetry<GetQueryResultsResponse | void>(
    async () => {
      const res = await cwl
        .getQueryResults({ queryId: id as string })
        .promise();
  ...
  const resultsResponse = r as GetQueryResultsResponse;
  return resultsResponse?.results?.length
    ? `${a.NewStateReason}${
        resultsResponse.results.map(
          (x) =>
            `\n${
...
const groups = [
  {
    functionName: (t: ITrigger): string | undefined =>
      t.Dimensions.filter((d) => d.name === "FunctionName").map(
        (d) => d.value
      )[0],
    match: (t: ITrigger) =>
